            ...
            <div class="col-md-4">
                <div class="card">
                    ...
                    <div class="card-body mt-4 ml-2">
                      <h5 class="card-title text-left"><small><i class="fas fa-user"></i> {{ ucfirst($profile->name) }}</small></h5>
                      <p class="card-text text-left"><small><i class="fas fa-envelope"></i> {{ $profile->email }}</small></p>
                      <p class="card-text text-left">
                          <small class="text-muted"><i class="fas fa-bookmark"></i>
                              @if($profile->role == 'user_pro')
                ...
                <form class="border px-4 pt-2 pb-3" method="POST" action="{{route('user-profile-update',$profile->id)}}">
                    ...
                    <div class="form-group">
                      ...
                      <div class="form-group">
                          ...
                          <select name="role" class="form-control" {{ ($profile->role == 'user' || $profile->role == 'user_pro')? 'disabled' : '' }}>
                              <option value="">-----Select Role-----</option>
                                  <option value="admin" {{(($profile->role=='admin')? 'selected' : '')}}>Admin</option>
                                  <option value="user_pro" {{(($profile->role=='user_pro')? 'selected' : '')}}>Professional User</option>
